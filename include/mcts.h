...
struct MCTSTree {
    ...
    MCTSTree(size_t tree_size, GlobalContext* ctx) 
        : _nodes(std::make_unique<MCTSNode<T>[]>(tree_size))
        , _ctx(ctx)
    ...
    void reset() {
        ...
        _ctx->totalMoves = 0;
        _ctx->totalSims = 0;
        _ctx->status = OverviewStatus::Active;

        _nodes[0].reset();
        unallocated_index.store(1, std::memory_order_relaxed);
        _game_over.store(false, std::memory_order_relaxed);
        root_node  = &_nodes[0];
        #ifdef NEURAL_ENABLED
        _game_history.clear();
        #endif
    ...
    std::span<MCTSNode<T>> create_children(MCTSNode<T>* parent, size_t count) {
        ...
        assert(start + count <= _tree_size && "Node pool exhausted");
    ...
    void register_simulation() {
        ...
        if (current >= SIMULATIONS_PER_MOVE) {
            ...
            if (_advancing.compare_exchange_strong(expected, true, std::memory_order_acq_rel)) {
                _move_simulation_count.store(0, std::memory_order_relaxed);
                advance_root();
                _advancing.store(false, std::memory_order_release);
            }
        }
    }
    ...
    bool advance_root() {
        ...
        if (!root_node || T::is_terminal(root_state) || root_node->child_count == 0 ) {
            _game_over.store(true, std::memory_order_relaxed);
            return false;
        };
        ...
        for (MCTSNode<T>& child : root_node->children) {
            float probability = (total_visits == 0) ? 0.0f : child.visit_count.load(std::memory_order_relaxed) / total_visits;
            data_sample.policy[child.action] = (probability > 1.0f) ? 1.0f : probability;
        }
    ...
    std::vector<LabelledData<T>> get_game_history() {
        ...
        for (int i = (int)_game_history.size() - 1; i >= 0; i--) {
            _game_history[i].value = outcome;
